...
class MatchDao
{
	...
	{
		bdd.getCnx().request("insert into test.Player (matchHistoryUri, profileIcon, summonerId, summonerName, participantId, matchId) values ('" + matchHistoryUri +"'," + profileIcon +"," + summonerId +",'" + summonerName +"'," + participantId +"," + matchId + ")");
	}
	...
	{
		bdd.getCnx().request("insert into test.Participant (championId, highestAchievedSeasonTier, participantId, spell1Id, spell2Id, teamId, matchId) values (" + championId +",'" + highestAchievedSeasonTier +"'," + participantId +"," + spell1Id +"," + spell2Id +"," + teamId +"," + matchId + ")");
	}
	
	public function insertMastery(masteryId:Int, rank:Int, participantId:Int, matchId:Int)
	{
		bdd.getCnx().request("insert into test.Mastery (masteryId, rank, participantId, matchId) values (" + masteryId +"," + rank +"," + participantId +"," + matchId + ")");
	}
	
	public function insertRune(runeId:Int, rank:Int, participantId:Int, matchId:Int)
	{
		bdd.getCnx().request("insert into test.Rune (rank, runeId, participantId, matchId) values (" + rank +"," + runeId +"," + participantId +"," + matchId + ")");
	}
	...
	{
		bdd.getCnx().request("insert into test.ParticipantTimeline (tenToTwenty, thirtyToEnd, twentyToThirty, zeroToTen, name, lane, role, participantId, matchId) values (" + tenToTwenty + "," + thirtyToEnd + "," + twentyToThirty + "," + zeroToTen + ",'" + name + "','" + lane + "','" + role + "'," + participantId + "," + matchId + ")");
	}
